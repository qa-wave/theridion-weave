...
import type { ResultStatus, RunSource, RunWorkflowStatus, ScriptStatus, TestCasePriority, TestCaseStatus } from "@/lib/types";
...
const scriptStatusColors: Record<ScriptStatus, string> = {
  draft: "bg-slate-500/15 text-slate-300 border-slate-500/30",
  active: "bg-emerald-500/15 text-emerald-300 border-emerald-500/30",
  flaky: "bg-orange-500/15 text-orange-300 border-orange-500/30",
  deprecated: "bg-zinc-600/20 text-zinc-400 border-zinc-600/30",
};

export function ScriptStatusBadge({ status }: { status: ScriptStatus }) {
  return (
    <span className={cn("inline-block rounded-md border px-2 py-0.5 text-xs font-medium", scriptStatusColors[status])}>
      {status}
    </span>
  );
}

const runStatusColors: Record<RunWorkflowStatus, string> = {
  ...
  signed_off: "bg-emerald-500/15 text-emerald-300 border-emerald-500/30",
  blocked: "bg-red-500/15 text-red-300 border-red-500/30",
};

export function RunStatusBadge({ status }: { status: RunWorkflowStatus }) {
  return (
    <span className={cn("inline-block rounded-md border px-2 py-0.5 text-xs font-medium", runStatusColors[status])}>
      {status.replace("_", " ")}
    </span>
  );
}
